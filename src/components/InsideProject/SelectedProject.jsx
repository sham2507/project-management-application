        ...
        <div className="w-[35rem] mt-16 animate-bounceOnce">
            <header className="pb-4 md-4 border-b-2 border-stone-300">
                <div className="flex items-center justify-between">
                    ...
                    <button onClick={onDelete} className="text-stone-600 hober:text-stone-950">Delete</button>
                </div>                
                <p className="mb-4 text-stone-400">{formattedDate}</p>
                <p className="text-stone-600 whitespace-pre-wrap">{project.description}</p>
            </header>
            <Tasks tasks={tasks} onAddTask={onAddTask} onDeleteTask={onDeleteTask} selectedProjectId={selectedProjectId}/>
        </div>
    );
}